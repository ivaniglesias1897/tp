...
@section('content')
    ...
    <div class="content px-3">
        ...
        <div class="card tabla-container" id="departamentos-table-container">
            @include('departamentos.table')
        </div>
    </div>

    {{-- === AGREGADO: MODAL DE HISTORIAL (Igual que en Cargos) === --}}
...
<script>
    $(document).ready(function() {
        ...
        function loadTable(url) {
            $.ajax({
                ...
                success: function(data) {
                    $('#departamentos-table-container').html(data); 
                },
                error: function(xhr) {
        ...
        // --- 2. PAGINACIÓN ---
        $(document).on('click', '#departamentos-table-container .pagination a', function(e) {
            e.preventDefault();
        ...
        $('form[method="GET"]').on('submit', function(e) {
             if ($(this).find('input[name="buscar"]').length > 0 && 
                 ...
                 var url = '{{ route('departamentos.index') }}' + '?buscar=' + buscar;
                 loadTable(url);
             }
        });

        // --- 4. LÓGICA DE HISTORIAL (NUEVO) ---
        $('body').on('click', '.btn-historial', function() {
            var id = $(this).data('id');
            var tabla = $(this).data('tabla'); // 'departamentos' o 'cargos'
