    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    @can('cajas create')
                        <a class="btn btn-primary float-right" href="{{ route('cajas.create') }}">
...
<script>
    $(document).ready(function() {
        ...
        function loadTable(url) {
            $.ajax({
                ...
                success: function(data) {
                    $('#cajas-table-container').html(data); 
                },
                error: function(xhr) {
        ...
        // --- 2. PAGINACIÓN ---
        $(document).on('click', '#cajas-table-container .pagination a', function(e) {
            e.preventDefault();
        ...
        $('form[method="GET"]').on('submit', function(e) {
             if ($(this).find('input[name="buscar"]').length > 0 && 
                 ...
                 var url = '{{ route('cajas.index') }}' + '?buscar=' + buscar;
                 loadTable(url);
             }
        });
        ...
        $('body').on('click', '.btn-historial', function() {
            var id = $(this).data('id');
            var tabla = $(this).data('tabla'); // 'cajas'
